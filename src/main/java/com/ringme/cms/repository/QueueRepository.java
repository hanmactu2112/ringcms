...
public interface QueueRepository extends JpaRepository<Queue, Long> {
    ...
    @Query(name = "select * from queue q where q.department_id =?1 and q.type_queue =?2 and and q.mission_id =?3  ", countQuery = "Select count(*) from queue q q.department_id =?1 and q.type_queue =?2 and and q.mission_id =?3 ", nativeQuery = true)
